...
class PowithetaExportThisYear implements FromCollection, WithHeadings, ShouldAutoSize, WithStyles, WithColumnFormatting, WithEvents
{
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                // Add quarter grouping and subtotals by project
                $this->addQuarterGrouping($sheet, $highestRow, $highestColumn);
            },
        ];
    }
    
    private function addQuarterGrouping($sheet, $highestRow, $highestColumn)
    {
        // Set column width for dates
        $sheet->getColumnDimension('V')->setWidth(15); // PO Delivery Date
        
        // Add totals row at the bottom with formula
        $totalsRow = $highestRow + 2;
        $sheet->setCellValue('A' . $totalsRow, 'GRAND TOTAL');
        $sheet->mergeCells('A' . $totalsRow . ':P' . $totalsRow);
        ...
        $sheet->setCellValue('Q' . $totalsRow, '=SUM(Q2:Q' . $highestRow . ')');
        $sheet->setCellValue('R' . $totalsRow, '=SUM(R2:R' . $highestRow . ')');
        $sheet->setCellValue('S' . $totalsRow, '=SUM(S2:S' . $highestRow . ')');
        
        // Apply number format to total cells
        $sheet->getStyle('Q' . $totalsRow . ':S' . $totalsRow)->getNumberFormat()
